 ...
 * Signature: ()I
 */
JNIEXPORT jint JNICALL Java_com_topeet_serialtest_serial_Open(JNIEnv *env, jobject obj, jint Port, jint Rate)
{
  if(fd <= 0)
  {
		if(0 == Port)
		...
		{
			__android_log_print(ANDROID_LOG_INFO, "serial", "open fd /dev/tty0");
			fd=open("/dev/tty0",O_RDWR|O_NDELAY|O_NOCTTY);
		}
		else
		{
 ...
 * Signature: ()I
 */
JNIEXPORT jint JNICALL Java_com_topeet_serialtest_serial_Close(JNIEnv *env, jobject obj)
  {
	if(fd > 0)close(fd);
  }
 ...
 * Signature: ()[I
 */
JNIEXPORT jintArray JNICALL Java_com_topeet_serialtest_serial_Read(JNIEnv *env, jobject obj)
 {
 ...
 * Signature: ()[I
 */
JNIEXPORT jintArray JNICALL Java_com_topeet_serialtest_serial_Write(JNIEnv *env, jobject obj, jintArray buf, jint buflen)
 {
	jsize len = buflen;
